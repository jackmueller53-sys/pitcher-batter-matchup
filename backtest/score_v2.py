...
def main(tag='v2', limit=None):
    ...
    pred_probs, actuals = [], []
    used_splits = used_whiff = used_park = used_relief = 0
    ...
    for row in pa.itertuples(index=False):
        ...
        if u['pitch_bump'] != 1.0: used_whiff += 1
        if u['park_hr'] != 1.0: used_park += 1
        if u['is_relief']:   used_relief += 1
    ...
    print(f'  features used: splits={used_splits:,} ({used_splits/n:.1%}) '
          f'whiff={used_whiff:,} ({used_whiff/n:.1%}) '
          f'park={used_park:,} ({used_park/n:.1%}) '
          f'relief={used_relief:,} ({used_relief/n:.1%})')
    ...
    with open(report, 'w') as f:
        ...
        f.write(f'- reliever bump applied: {used_relief/n:.1%}\n\n')
